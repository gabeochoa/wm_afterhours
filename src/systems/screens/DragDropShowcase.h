...
struct DragDropShowcase : ScreenSystem<UIContext<InputAction>> {
  ...
  void render_card(UIContext<InputAction> &context, afterhours::Entity &parent,
    ...
    div(context, mk(parent, id),
        ComponentConfig{}
            ...
            .with_custom_background(afterhours::Color{62, 65, 76, 255})
  ...
  void render_column(UIContext<InputAction> &context,
                     afterhours::Entity &parent, int id,
                     const std::string &title, std::vector<Card> &items,
                     const Theme &theme, int column_index) {
    // Column container
    auto column =
        vstack(context, mk(parent, id),
               ComponentConfig{}
                   .with_size(ComponentSize{percent(0.30f), percent(1.0f)})
                   .with_custom_background(afterhours::Color{42, 44, 54, 255})
    ...
    if (items.empty()) {
      auto empty_box =
          hstack(context, mk(cards_area.ent(), 999),
                 ComponentConfig{}
      ...
      div(context, mk(empty_box.ent(), 0),
          ComponentConfig{}
              ...
              .with_custom_text_color(afterhours::Color{120, 120, 135, 255})
  ...
  void for_each_with(afterhours::Entity &entity,
    ...
    if (drag_state) {
      for (auto &evt : drag_state->events) {
        ...
        if (src_it == group_to_column.end() || tgt_it == group_to_column.end())
          continue;
