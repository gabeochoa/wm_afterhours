...
struct ExampleAccessibility : afterhours::System<UIContext<InputAction>> {
    ...
    void for_each_with(afterhours::Entity &entity,
        ...
        if (!validated_theme) {
            theme_is_accessible =
                ThemeDefaults::get().validate_theme_accessibility();
        ...
        afterhours::Color status_color =
            theme_is_accessible ? afterhours::Color{50, 150, 50, 255}
        ...
        for (int i = 0; i < 4; i++) {
            button(context, mk(entity, 30 + i),
                   ComponentConfig{}
                       .with_label("Auto Contrast")
                       .with_size(ComponentSize{pixels(130.0f),
                                                pixels(button_height)})
                       .with_absolute_position()
