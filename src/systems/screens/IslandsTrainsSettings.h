...
struct IslandsTrainsSettingsScreen : ScreenSystem<UIContext<InputAction>> {
  ...
  afterhours::Color bg_olive_dark{110, 125, 100, 255};   // Darker olive for edges (stronger)
  afterhours::Color bg_olive_mid{150, 162, 135, 255};    // Mid olive-sage 
  afterhours::Color bg_olive_light{170, 180, 155, 255};  // Lighter olive for center
  
  // Panel and UI elements
  afterhours::Color panel_cream{252, 252, 245, 255};     // Warm cream panel (more opaque)
  afterhours::Color row_cream{235, 240, 228, 255};       // Row background - soft sage-cream
  afterhours::Color text_dark{70, 80, 75, 255};          // Dark gray-green text for labels
  afterhours::Color text_muted{145, 155, 145, 255};      // Muted text for secondary elements
  afterhours::Color header_olive{95, 135, 115, 255};     // Section header - more saturated teal-olive
  afterhours::Color slider_teal{90, 155, 140, 255};      // Slider filled - more saturated teal
  afterhours::Color slider_empty{205, 175, 170, 255};    // Slider empty - dusty rose/mauve
  ...
  void for_each_with(afterhours::Entity &entity,
    ...
    ui_workarounds::vertical_gradient(
        context, entity, 1,
        0.0f, 0.0f, sw, sh * 0.3f,
        bg_olive_dark, bg_olive_mid, 8);

    // Bottom edge gradient (darker at bottom)
    ui_workarounds::vertical_gradient(
        context, entity, 10,
        0.0f, sh * 0.7f, sw, sh * 0.3f,
        bg_olive_mid, bg_olive_dark, 8);

    // Left edge gradient (darker at left)
    ui_workarounds::horizontal_gradient(
        context, entity, 20,
        0.0f, 0.0f, sw * 0.2f, sh,
        afterhours::Color{bg_olive_dark.r, bg_olive_dark.g, bg_olive_dark.b, 200},
        afterhours::Color{bg_olive_mid.r, bg_olive_mid.g, bg_olive_mid.b, 0}, 6);

    // Right edge gradient (darker at right)
    ui_workarounds::horizontal_gradient(
        context, entity, 30,
        sw * 0.8f, 0.0f, sw * 0.2f, sh,
        afterhours::Color{bg_olive_mid.r, bg_olive_mid.g, bg_olive_mid.b, 0},
        afterhours::Color{bg_olive_dark.r, bg_olive_dark.g, bg_olive_dark.b, 200}, 6);
    ...
    div(context, mk(entity, 50),
        ComponentConfig{}
            ...
            .with_soft_shadow(4.0f, 6.0f, 20.0f, afterhours::Color{50, 60, 50, 30})
    ...
    // Cam panning speed
    render_slider_row(context, entity, 140, content_x, controls_row_y, row_w, row_h,
                      "Cam panning speed", cam_pan_speed, 10);
    
    // Cam rotating speed
    render_slider_row(context, entity, 160, content_x, controls_row_y + row_h + section_gap, row_w, row_h,
    ...
    button(context, mk(entity, 180),
           ComponentConfig{}
               ...
               .with_absolute_position()
               .with_translate(content_x, controls_row_y + 2 * (row_h + section_gap))
    ...
    // Effects Volume
    render_slider_row(context, entity, 200, content_x, audio_row_y, row_w, row_h,
                      "Effects Volume", effects_volume, 10);
    
    // Music Volume
    render_slider_row(context, entity, 220, content_x, audio_row_y + row_h + section_gap, row_w, row_h,
    ...
    for (int i = 0; i < max_val; i++) {
      ...
      if (button(context, mk(entity, base_id + 10 + i),
          ComponentConfig{}
              ...
              .with_absolute_position()
              .with_translate(slider_start_x + static_cast<float>(i) * (seg_w + seg_gap), y + 11.0f)
