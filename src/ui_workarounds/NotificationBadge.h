...
inline void notification_badge(
    Context &context, Entity &entity, int id,
    const std::string &text, // "2", "!", etc.
    float x, float y, float size = 20.0f,
    afterhours::Color bg_color = afterhours::Color{180, 60, 60, 255}, // Red
    afterhours::Color text_color = afterhours::Color{255, 255, 255, 255}
    // White
) {
...
inline void icon_button_with_badge(
    Context &context, Entity &entity, int base_id,
    const std::string &icon_text, // Symbol to show in the icon
    const std::string &label,     // Label below the icon
    float x, float y, float icon_size, afterhours::Color icon_bg,
    afterhours::Color icon_border, afterhours::Color icon_text_color,
    afterhours::Color label_color, const std::string &font_name,
    float icon_font_size, float label_font_size,
  ...
  if (!badge_text.empty()) {
    notification_badge(context, entity, base_id + 2, badge_text,
                       x + icon_size - 12.0f, // Position at top-right of icon
                       y - 5.0f, 18.0f, badge_bg);
  }
}
