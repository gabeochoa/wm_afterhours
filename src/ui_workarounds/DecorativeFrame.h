...
// multiple overlapping border rectangles to create a kraft paper / scrapbook
// edge effect.
//
...
inline void
kraft_paper_frame(Context &context, Entity &entity, int base_id, float screen_w,
                  float screen_h,
  ...
  div(context, mk(entity, base_id + 1),
      ComponentConfig{}
          .with_size(
              ComponentSize{pixels(static_cast<int>(screen_w - inset1 * 2)),
  ...
  div(context, mk(entity, base_id + 2),
      ComponentConfig{}
          .with_size(
              ComponentSize{pixels(static_cast<int>(screen_w - inset2 * 2)),
  ...
  div(context, mk(entity, base_id + 6),
      ComponentConfig{}
          .with_size(ComponentSize{pixels(static_cast<int>(corner_size)),
                                   pixels(static_cast<int>(corner_size))})
          .with_absolute_position()
          .with_translate(screen_w - corner_size - 2.0f,
                          screen_h - corner_size - 2.0f)
          .with_custom_background(corner_color)
